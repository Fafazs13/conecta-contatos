...
function listarContatos(arrayContatos = []) {
    ...
    if(contatos.length < 1) {
        contContatos.inneText = 'nenhum Contato'
        return
    } if(contatos.length > 1)  {
        contContatos.innerText = `${contatos.length} contato`
    }
...
function deletarContato(index) {
    const contatos = pegarDadosLocalStorage() // 10
...
document.getElementById('pesquisar').addEventListener('keyup', (event) => {
      const contatos = pegarDadosLocalStorage()
      const pesquisar = event.target.value.toLowerCase()

      const contatosFiltrados = contatos.filter(contato => {
        return contato.nome.toLowerCase().includes(pesquisar)
      })
